...
(() => {
  ...
  function batch(fn) {
    ...
    markRecent(box);
    if (stick) {
      // Desplazamiento suave: el texto sube de a poco en vez de saltar.
  ...
  function upsert(cap, announce) {
    ...
    if (!flow || !current || cap.session !== current.sid || cap.channel !== current.ch) return;
    document.getElementById("waiting")?.remove();
    let el = lines.get(cap.seg);
    if (el && el.dataset.final === "1" && !cap.final) return; // parcial atrasado
    if (!el) {
      // Cada oración es un tramo dentro de un texto continuo: cerrar una oración no mueve
      // nada de lugar. Si hubo una pausa, la oración empieza un párrafo nuevo.
      el = document.createElement("span");
      el.className = "line";
      el.dataset.seg = cap.seg;
      el.dataset.t0 = cap.t0;
      let before = null;
      for (let n = flow.lastElementChild; n && n.dataset.seg; n = n.previousElementSibling) {
        if (Number(n.dataset.seg) > cap.seg) before = n; else break;
      }
      const prev = before ? before.previousElementSibling : flow.lastElementChild;
      if (prev && Number(cap.t0) - Number(prev.dataset.t1 || prev.dataset.t0) > 1.5) el.classList.add("para");
  ...
  // El texto se atenúa recién cuando quedó atrás: más de 4 oraciones arriba y más de 20 s
  // desde que se cerró. Así no se "nubla" lo que la persona todavía está leyendo.
  function markRecent(box) {
    const all = [...box.querySelectorAll(".line")];
    const now = Date.now();
    all.forEach((n, i) => {
      if (n.dataset.final === "1" && !n.dataset.doneAt) n.dataset.doneAt = now;
      const old = i < all.length - 4 && n.dataset.doneAt && now - Number(n.dataset.doneAt) > 20000;
      n.classList.toggle("recent", !old);
    });
  }
  setInterval(() => { const b = document.getElementById("captions"); if (b) markRecent(b); }, 5000);
