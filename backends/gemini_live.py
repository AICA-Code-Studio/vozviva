...
class GeminiLiveBackend(_LiveBase):
    ...
    async def handle(self, msg: Any, ctx: _Ctx) -> None:
        ...
        if interim is not None and interim.text:
            if ctx.seg is None:
                ctx.seg = self.next_seg()
                ctx.t0 = self.t
            ctx.lang = _short_lang(interim.language_code) or ctx.lang
            ctx.interim = interim.text
            await self.emit(TranscriptEvent(
                seg=ctx.seg, text=(ctx.final_buf + interim.text).strip(), final=False,
                t0=ctx.t0, t1=self.t, lang=ctx.lang or self.session.source_lang,
            ))
    ...
    async def _commit(self, ctx: _Ctx) -> None:
        text = ctx.final_buf.strip()
        seg = ctx.seg
        ctx.final_buf, ctx.interim, ctx.seg = "", "", None
        # Fin de la frase: el último silencio que detectó el VAD local dentro del segmento.
        t1 = self.speech_end_t if ctx.t0 <= self.speech_end_t <= self.t else self.t
        if text and seg is not None and self.emit is not None:
            await self.emit(TranscriptEvent(
                seg=seg, text=text, final=True, t0=ctx.t0, t1=t1,
                lang=ctx.lang or self.session.source_lang,
            ))

    async def close_ctx(self, ctx: _Ctx) -> None:
        # si la conexión se cerró con un parcial en pantalla, lo cerramos con lo que había
        if not ctx.final_buf and ctx.interim:
            ctx.final_buf = ctx.interim
        if ctx.final_buf:
            await self._commit(ctx)
        ctx.seg = None
